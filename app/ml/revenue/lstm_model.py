...
class LSTMForecaster:
    ...
    def _build_model(self) -> Sequential:
        """Xây dựng kiến trúc mô hình LSTM với Input layer"""
        model = Sequential([
            Input(shape=(self.look_back, 1)),  # Thêm Input layer thay vì input_shape trong LSTM
    ...
    def train(self, historical_data: pd.DataFrame):
        self.max_date = historical_data['TransactionDate'].max()
    ...
    def predict(self, last_values: np.ndarray, start_date, end_date) -> np.ndarray:
        ...
        for _ in range(periods):
            current_pred = self.model.predict(current_batch, verbose=0)[0]  # Tắt verbose để log gọn hơn
